...
class Controller:
    def __init__(self, args):
        ...
        log_name = f"{self.target_fps}FPS_{self.K_p}Kp_{self.K_d}Kd_{self.CONTAINER_CAPACITY}Capacity_{self.target_model_size}ModelSize"
    ...
    def run(self):
        ...
        while True:
            ...
            while True:
                ...
                # if "processing_rates" in sensor:
                #     self.processing_rate[sensor] = (timestamp, value)
            ...
            if now - last_control >= 2:
                ...
                processing_rates = [value for _, (ts, value) in self.processing_rates.items() if now - ts <= 5]
                ...
                total_needed, error, control_signal = self.pid_control(
                    error=frames_needed_to_process - total_processing_rate)
